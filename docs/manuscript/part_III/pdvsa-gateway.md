...
![The lattice-linear flow ramp of the EGS Lattice-Linear Gateway: cumulative flow $L_n = n\,r$ rises in equal increments across the ops decision cycle (Gather → Decide → Act), generated from `textbook.models.lattice_linear_profile`.](../../output/figures/part_III_pdvsa-gateway.png){#fig:part_III_pdvsa-gateway width=90%}
